...
public class LoadChat extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for (Chat chat : chat_list) {
            ...
            chatObject.addProperty("from_user_id", chat.getFrom_user().getId());
            chatObject.addProperty("to_user_id", chat.getTo_user().getId());

            // Determine the side based on the user
            if (chat.getFrom_user().getId() == other_user.getId()) {
                chatObject.addProperty("side", "left");

                // Update status if necessary
                if (chat.getChat_status().getId() == 2) {
                    chat.setChat_status((Chat_Status) session.get(Chat_Status.class, 1)); // Set status to "sent"
                    session.update(chat);
                }
            } else {
